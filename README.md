...
| `crates/civitas` | **The community of model-driven people**, over the same transition function and the same audit: one model session per person, each holding one member, seeing only what a wallet sees and acting through tools that compose the swarm's own intents. Where the swarm's strategies are written, these are not: what a person does with their day is the model's, and what the ledger does about it is arithmetic. A run writes a tape — typed events beside the raw exchange, and a manifest naming the model and the sha256 of what each tier was told — which `replay` re-derives with no model call. The world beside the ledger is seeded and configured: households scaled to who a person is, a ledger not everybody has adopted, and an acquaintance graph, each absent from a manifest meaning the world without it, so an old tape still replays. **Run by hand, never by `ci`**: a gate that calls a model is a gate whose verdict is a sample |
| `ui/player` | Svelte reader for an indexed run: the stake and contract graph a day at a time, a person's day beside the ledger state it produced, fifteen economic tracks, twenty-six anomaly studies, and every contract drilled to the offer that opened it. It reads what `civitas index` writes and nothing else (`just civitas-shape-check`), and the ledger's refusal codes arrive from the wallet's own `en.json` at build time, so it cannot drift from what a member is told |
...
# A community of model-driven people over the real ledger. BY HAND, never in a
# gate, and every one of these but the first two spends money at a model API:
just civitas-scripted <cfg> <dir>   # the apparatus alone: no model call, no cost
just civitas-read <dir>             # replay a tape, index it, write violations.md and queue.md
just civitas-run <cfg> <dir>        # a sitting; `civitas-resume <dir> <days>` continues one
just civitas-prompt <cfg> <dir>     # what a person carries, and what the endpoint counted for it
just civitas-player                 # open an indexed run
...
| `crates/state` | 64 + 312 across twenty suites, each re-auditing all seven invariants over sets after every transition, accepted or refused, and holding the write gate's memo to the definition's verdict |
...
| `ui/wallet` | 349 tests across 37 suites, svelte-check clean, production build clean |
| `ui/player` | 14 tests, and a shape gate pinning every key it reads to what `civitas index` writes |
| `crates/civitas` | **No tests, by decision**: nothing that calls a model belongs in a gate. `ci` compiles and lints it and asserts nothing about what it does; what a run leaves is a tape, and `replay` re-deriving it is the check |
