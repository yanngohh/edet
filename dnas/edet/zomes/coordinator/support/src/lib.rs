...
#[hdk_extern]
pub fn get_sys_time() -> ExternResult<Timestamp> {
    sys_time()
}
    ...
    match action.hashed.content.clone() {
        ...
        Action::DeleteLink(delete_link) => {
            let record = get(delete_link.link_add_address.clone(), GetOptions::default())?
                .ok_or(wasm_error!(WasmErrorInner::Guest("Failed to fetch CreateLink action".to_string())))?;
            match record.action() {
                Action::CreateLink(create_link) => {
                    if let Ok(Some(link_type)) = LinkTypes::from_type(create_link.zome_index, create_link.link_type) {
                ...
                _ => Err(wasm_error!(WasmErrorInner::Guest("Create Link should exist".to_string()))),
            }
        }
