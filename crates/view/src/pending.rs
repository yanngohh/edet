...
//!
//! **What a signature is, and which bytes it signs, is the caller's.** The
//! pool asks a [`Scheme`] for both at every place it checks one — the door's
//! `verified_signer`, `sign`, `decline` and `invited_by` — so the checks stay
//! where they are and one rule is enforced whoever calls. The node passes the
//! envelope digest and Ed25519. A simulation, which holds no keys, passes a
//! scheme under which a signature proves only which session called; what the
//! pool derives from a signature is, there, a property of that binding.
...
/// The signing scheme a pool's caller runs under: the bytes an envelope signs,
/// and whether a signature over some bytes verifies.
pub trait Scheme {
    /// The digest a signer of this envelope signs, or `None` if it does not
    /// encode.
...
/// A completed entry: the transaction, its envelope fields, and every verified
/// signature in key order. The caller assembles its own envelope from it.
#[derive(Clone, Debug)]
...
pub struct Invite {
    ...
    pub nonce: [u8; 16],
    /// The inviter's signature over `invite_message`, under the caller's
    /// [`Scheme`] — Ed25519 on a node.
    pub signature: Vec<u8>,
}
...
///
/// Mirrored client-side by `ui/wallet/src/lib/invite.ts::inviteMessage`; both are
/// pinned against the same fixed vector (the Rust test in this module and
/// `ui/wallet/src/lib/__tests__/invite.test.ts`), so a change on either side
...
///
/// Mirrored client-side by `ui/wallet/src/lib/submit.ts::declineMessage`; the two
...
/// this module and the vitest `ui/wallet/src/lib/__tests__/decline-message.test.ts`
...
/// is one rule.
pub fn invited_by(scheme: &dyn Scheme, state: &State, req: &PendingSignReq, now_secs: u64) -> Option<(MemberId, Key)> {
...
pub fn verified_signer(scheme: &dyn Scheme, state: &State, req: &PendingSignReq, digest: &[u8; 32]) -> Option<Party> {
    if !scheme.verify(&req.signer, digest, &req.signature) {
        return None;
    }
...
pub fn request_digest(scheme: &dyn Scheme, state: &State, req: &PendingSignReq) -> Option<[u8; 32]> {
    scheme.digest(&state.chain_id, &req.tx, &req.nonce, req.not_after_epoch)
}
...
pub enum SignOutcome {
    ...
    /// Threshold reached: the transaction and its signatures, for the caller
    /// to assemble into its envelope and submit. The entry is tombstoned.
    ///
...
impl PendingPool {
    ...
    /// counts.
    pub fn sign(&mut self, scheme: &dyn Scheme, state: &State, req: PendingSignReq, now_secs: u64) -> SignOutcome {
        self.prune(now_secs);
        let Some(digest) = request_digest(scheme, state, &req) else {
        ...
        let Some(party) = verified_signer(scheme, state, &req, &digest) else {
        ...
        let (member, opener) = match (party, self.entries.contains_key(&digest)) {
            ...
            (Party::Key(k), false) => match invited_by(scheme, state, &req, now_secs) {
                Some((inviter, _)) => {
                    let open = self
                        .entries
                        .values()
        ...
        if signed_parties(state, entry).len() >= entry.min_sigs {
            ...
            return SignOutcome::Complete(Box::new(Completed {
                tx: entry.tx,
                nonce: entry.nonce,
    ...
    pub fn decline(
        ...
        if !scheme.verify(signer, &decline_message(&digest), signature) {
            return false;
        }
    ...
    /// How many entries are still inside their window at `epoch`. An entry
    /// past it is not waiting for anybody: the ledger answers its signature
    /// with `ET-TX-002`, and a count that carried it would say more was
    /// waiting than could ever go through.
    pub fn live_count(&self, epoch: u64) -> usize {
        self.entries.values().filter(|e| e.not_after_epoch >= epoch).count()
    }
...
mod tests {
    ...
    /// Cross-pin with `ui/wallet/src/lib/invite.ts::inviteMessage` and its vitest:
    ...
    /// Cross-pin with `ui/wallet/src/lib/submit.ts::declineMessage` and its vitest
    /// (`ui/wallet/src/lib/__tests__/decline-message.test.ts`): both compute
