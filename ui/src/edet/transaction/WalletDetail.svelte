<script lang="ts">
    ...
    let activeAsDebtor: number = 0;
    let activeAsCreditor: number = 0;
    ...
    async function fetchMetrics() {
        ...
        try {
            ...
            activeAsDebtor = debtorContracts.length;
            activeAsCreditor = creditorContracts.length;
        } catch (e: any) {
...
{#if wallet !== undefined}
    <div class="wallet-card card">
        <div class="card-header flex-row align-vcenter" data-tour="wallet-header">
            ...
            {#if showActions}
                <div class="header-actions flex-row" data-tour="wallet-qr">
        ...
        <div class="metrics-grid" data-tour="wallet-metrics" data-loaded={metricsLoaded ? 'true' : undefined}>
            <div class="metric-item card elevation-z1">
                ...
                <div class="metric-help" tabindex="0" role="button"
            ...
            <div class="metric-item card elevation-z1">
                <div class="metric-header">
                    <i class="material-icons metric-icon connections" aria-hidden="true">people</i>
                    <div class="metric-body">
                        <span class="metric-label">{$_("walletDetail.connections_label", {default: "Connections"})}</span>
                        <span class="metric-value">{Math.max(0, acquaintanceCount - 1)}</span>
                    </div>
                </div>
                <div class="metric-help" tabindex="0" role="button"
                    aria-label={$_("walletDetail.connections_tooltip")}
                    data-tooltip={$_("walletDetail.connections_tooltip")}>
                    <i class="material-icons" aria-hidden="true">help_outline</i>
                </div>
                <div class="progress-spacer"></div>
            </div>
            ...
            <div class="metric-item card elevation-z1">
                <div class="metric-header">
                    <i class="material-icons metric-icon transactions" aria-hidden="true">receipt_long</i>
                    <div class="metric-body">
                        <span class="metric-label">{$_("walletDetail.transactions_label", {default: "Transactions"})}</span>
                        <span class="metric-value trial-value">
                            <span class="trial-part">{activeAsDebtor} <span class="trial-sub">{$_("walletDetail.tx_active_out", {default: "active out"})}</span></span>
                            <span class="trial-sep">·</span>
                            <span class="trial-part">{activeAsCreditor} <span class="trial-sub">{$_("walletDetail.tx_active_in", {default: "active in"})}</span></span>
                        </span>
                        <span class="metric-sub">
                            {claim ? claim.cumulative_stats.total_contracts_processed : 0} {$_("walletDetail.tx_total_sent", {default: "sent"})}
                            · {claim ? claim.successful_transfers : 0} {$_("walletDetail.tx_settled", {default: "settled"})}
                        </span>
                    </div>
                </div>
                <div class="metric-help" tabindex="0" role="button"
                    aria-label={$_("walletDetail.transactions_tooltip")}
                    data-tooltip={$_("walletDetail.transactions_tooltip")}>
                    <i class="material-icons" aria-hidden="true">help_outline</i>
                </div>
                <div class="progress-spacer"></div>
            </div>
...
<style>
    ...
    :global(.metric-icon.connections) { color: #0288d1 !important; }
    :global(.metric-icon.transactions) { color: #5e35b1 !important; }
