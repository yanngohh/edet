<script lang="ts">
    ...
    /**
     * A seller named by key has no row, and this member's signature is what
     * would seat it — out of this member's own reach. Refused here, in the
     * wallet's words, rather than at a signature after the seller has been
     * told a trade was coming.
     */
    $: seatsSeller = me !== null && otherParty !== null && 'Key' in otherParty;
    $: noSeat = seatsSeller && !canSeat($myMember?.operation_bond);
...
>
    ...
    {:else}
        ...
        />
        {#if noSeat}
            <p class="no-seat" role="alert">
                {$_('transactions.noSeat', {
                    default: 'They have no account yet, and your standing carries no seat right now, so this purchase cannot open one. Settle it in money today, or try again once somebody has backed you further.',
                })}
            </p>
        {/if}

        <div class="page-actions">
            <Button variant="raised" disabled={submitting || otherParty === null || noSeat} on:click={submit}>
