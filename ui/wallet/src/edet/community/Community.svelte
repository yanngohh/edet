<script lang="ts">
    ...
    import { membersList, listTruncation, networkView, activeBase, refresh, contractsList } from '../../lib/node';
    import { dealingsWith, hasDealings } from '../../lib/dealings';
    ...
    // The ledger's own record, as counts: what the community has seen come
    // good and what stands unpaid. Read for trusting the ledger, not anybody.
    $: record = $networkView;
    /** The pieces of one member's dealings with me that are not zero. */
    function dealingParts(them: number): string[] {
        if (me === null) return [];
        const d = dealingsWith($contractsList, me, them);
        if (!hasDealings(d)) return [$_('community.dealingsNone', { default: 'You have never traded with them on edet.' })];
        const parts: string[] = [];
        if (d.paidMe > 0) parts.push($_('community.dealingsPaidMe', { values: { n: d.paidMe }, default: `they have paid you ${d.paidMe} times` }));
        if (d.pastDueToMe > 0) parts.push($_('community.dealingsPastDue', { values: { n: d.pastDueToMe }, default: `past due on ${d.pastDueToMe} debts to you` }));
        if (d.owedToMeOpen > 0) parts.push($_('community.dealingsOwedToMe', { values: { n: d.owedToMeOpen }, default: `they owe you ${d.owedToMeOpen} open` }));
        if (d.iPaid > 0) parts.push($_('community.dealingsIPaid', { values: { n: d.iPaid }, default: `you have paid them ${d.iPaid} times` }));
        if (d.iOweOpen > 0) parts.push($_('community.dealingsIOwe', { values: { n: d.iOweOpen }, default: `you owe them ${d.iOweOpen} open` }));
        return parts;
    }
...
<div class="main-container flex-column">
    ...
    {#if record}
        <p class="record">
            {$_('community.record', {
                values: {
                    accounts: record.members,
                    paid: record.contracts_paid,
                    defaulted: record.contracts_defaulted,
                    open: record.contracts_open,
                },
                default: `${record.members} accounts · ${record.contracts_paid} contracts paid through edet · ${record.contracts_defaulted} in default · ${record.contracts_open} open`,
            })}
        </p>
    {/if}
            ...
            <div class="member-row" class:me={m.id === me}>
                ...
                {#if expanded === m.id}
                    <div class="member-detail">
                        ...
                        {#if m.id !== me}
                            <p class="dealings">
                                <span class="k">{$_('community.dealings', { default: 'Dealings with you' })}:</span>
                                {dealingParts(m.id).join(' · ')}
                            </p>
                        {/if}
                        {#if detail}
