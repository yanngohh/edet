<script lang="ts">
    ...
    function remintInvite() {
        ...
        // **No invitation while the standing carries no seat.** A purchase
        // opened on it by a buyer with no account would be refused at this
        // member's signature, after the buyer had done everything asked of
        // them; the bare address still serves every buyer who has an account.
        invite = seed && chain && canSeat($myMember?.operation_bond) ? mintInvite(seed, chain, nowUnixSecs() + INVITE_TTL_SECS) : null;
    }
    ...
    // Named so the statement below re-runs when the seat flips, not on every
    // poll: an invitation minted under a seat is withdrawn when it is gone.
    $: seatKnown = canSeat($myMember?.operation_bond);
    $: if (seedKnown && chainKnown && typeof seatKnown === 'boolean') remintInvite();
    ...
    <div class="flex-column main-container">
        <div class="wallet-header" data-tour="wallet-header">
            ...
            <div class="wallet-qr" data-tour="wallet-qr">
                ...
                {#if seedKnown && chainKnown && !seatKnown}
                    <span class="qr-hint warn">
                        {$_('myWallet.qrNoSeat', {
                            default: 'Buyers who already have an account can use this. Nothing can open a new account until your standing carries a seat.',
                        })}
                    </span>
                {/if}
            </div>
...
<style>
    ...
    :global(.dark-theme) .qr-hint.warn {
        color: #ffcc80;
    }
    .qr-hint {
        font-size: 0.75rem;
