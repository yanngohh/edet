<script lang="ts">
    ...
    import type { TrustedMember } from '../../lib/policy';
    ...
    // The people signed on their name: a copy the form edits and `save`
    // writes whole, like every other field of the rule.
    let trusted: TrustedMember[] = $acceptancePolicy.trusted.map((t) => ({ ...t }));
    let trustMember: number | null = null;
    let trustAmountStr = '';
    $: me = $currentActorId;

    function addTrusted() {
        if (trustMember === null) return;
        const maxAmount = parseNumber(trustAmountStr);
        if (!(maxAmount > 0)) return;
        trusted = [...trusted.filter((t) => t.member !== trustMember), { member: trustMember, maxAmount }];
        trustMember = null;
        trustAmountStr = '';
    }

    function dropTrusted(member: number) {
        trusted = trusted.filter((t) => t.member !== member);
    }
...
<ActionPage icon="tune" title={$_('acceptance.title', { default: 'Acceptance rule' })} on:close={() => dispatch('close')}>
    ...
    <div class="ceiling" class:off={!auto}>
        <span class="k-label">{$_('acceptance.trustedTitle', { default: 'People signed on their name' })}</span>
        <Explain
            tone="notice"
            summary={$_('acceptance.trustedSummary', {
                default: 'A regular whose purchases you sign whatever they score, up to an amount.',
            })}
        >
            {$_('acceptance.trustedNote', {
                default:
                    'Somebody the community has not backed yet scores as a stranger for as long as nobody has carried them — even a customer you have known for years. Name them here and their purchases up to the amount are signed for you. The ledger still refuses what it would refuse, and nothing that would make you the debtor is ever signed.',
            })}
        </Explain>
        {#each trusted as t (t.member)}
            <div class="trusted-row">
                <MemberChip memberId={t.member} size={20} />
                <span class="trusted-amount">
                    {$_('acceptance.trustedUpTo', { values: { amount: formatNumber(t.maxAmount) }, default: `up to ${formatNumber(t.maxAmount)}` })}
                </span>
                <Button variant="outlined" disabled={!auto} on:click={() => dropTrusted(t.member)}>
                    <Label>{$_('common.remove', { default: 'Remove' })}</Label>
                </Button>
            </div>
        {/each}
        <div class="trusted-add">
            <AddressInput
                bind:value={trustMember}
                label={$_('acceptance.trustedAddress', { default: "Their address" })}
                exclude={[...(me === null ? [] : [me]), ...trusted.map((t) => t.member)]}
            />
            <NumericInput bind:value={trustAmountStr} label={$_('acceptance.trustedAmount', { default: 'Sign up to' })} />
            <Button variant="outlined" disabled={!auto || trustMember === null} on:click={addTrusted}>
                <Label>{$_('acceptance.trustedAdd', { default: 'Add' })}</Label>
            </Button>
        </div>
    </div>
