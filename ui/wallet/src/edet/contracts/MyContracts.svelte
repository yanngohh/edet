<script lang="ts">
    ...
    import { dueNow } from '../../lib/due';
    import { autopay, setAutopay } from '../../lib/autopay';
...
<div class="main-container flex-column">
    ...
    <!-- **What falls due, before the list.** The date is the ledger's and the
         member's attention is not: every default whose debtor held the cash
         was a debtor nobody had told. -->
    {#if $dueNow.length > 0}
        <div class="due-block" role="status">
            <div class="due-title">
                <i class="material-icons" aria-hidden="true">event</i>
                {$_('myContracts.dueTitle', { default: 'Falling due' })}
            </div>
            <ul class="due-list">
                {#each $dueNow as n (n.contract.id)}
                    <li class:past={n.state === 'past'}>
                        <span class="due-id">#{n.contract.id}</span>
                        <span class="due-amount">{formatNumber(n.contract.outstanding)}</span>
                        {#if n.contract.creditor !== undefined}
                            <MemberChip memberId={n.contract.creditor} size={18} />
                        {/if}
                        <span class="due-when">
                            {#if n.state === 'past'}
                                {$_('myContracts.duePast', { default: 'past due — pay it to cure the default' })}
                            {:else if n.left <= 0}
                                {$_('myContracts.dueToday', { default: 'due today' })}
                            {:else}
                                {$_('myContracts.dueIn', {
                                    values: { days: n.left, date: $epochDate(n.contract.maturity_epoch) },
                                    default: `due ${$epochDate(n.contract.maturity_epoch)}, in ${n.left} days`,
                                })}
                            {/if}
                        </span>
                    </li>
    ...
    <div class="list">
        {#each open as c (c.id)}
            ...
            >
                ...
                {#if c.status === 'active'}
                    <!-- The one standing instruction the wallet carries: the
                         debtor's half of a settlement, opened the day before.
                         The creditor still acknowledges it, under Requests. -->
                    <div class="autopay-row">
                        <FormField>
                            <Checkbox checked={$autopay.has(c.id)} on:click={() => setAutopay(c.id, !$autopay.has(c.id))} />
                            <span slot="label">{$_('myContracts.autopayLabel', { default: 'Pay at maturity' })}</span>
                        </FormField>
                        <span class="hint autopay-note">
                            {$_('myContracts.autopayNote', {
                                default: 'The day before this falls due, this device sends your creditor a payment request for what is outstanding; it is settled once they sign it. Works while the app is running, in front or in the background — nothing is sent once it is closed.',
                            })}
                        </span>
                    </div>
...
<style>
    ...
    .due-list li.past .due-when {
        color: #d32f2f;
        font-weight: 600;
    }
    :global(.dark-theme) .due-list li.past .due-when {
        color: #e57373;
    }
    .due-id {
