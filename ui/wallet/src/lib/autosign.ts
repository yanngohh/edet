...
import { acceptancePolicy, classify, trustedCeiling, type AcceptancePolicy, type Band } from './policy';
...
export async function processEntry(entry: PendingEntryView): Promise<AutoOutcome> {
    ...
    const scored = decideBand(row, policy, riskK(get(paramsView)), vBase(get(paramsView)), {
        member: debtor,
        pricing: get(subjectivePricing),
    });
    const risk = scored.risk;
    let band = scored.band;
    // **A trusted member is signed on their name up to their ceiling**, and
    // held above it like anybody else: the list says whom the member trusts,
    // the ceiling says how far, and neither moves the ledger's own verdict
    // below.
    const amount = grantedAmount(entry.tx);
    const ceiling = trustedCeiling(policy, debtor);
    if (ceiling !== null && amount !== null && amount <= ceiling) {
        band = 'accept';
    } else {
        if (band === 'hold') return 'left';
        // The ceiling binds only the SIGNING half. Declining a request too
        // large to sign automatically would be worse than holding it: the
        // score says the counterparty is bad, the ceiling says only that the
        // amount is large, and those are different sentences. A large request
        // from a well-scoring counterparty is the archetype of one its owner
        // should read.
        if (band === 'accept' && (amount === null || amount > policy.maxAmount)) return 'left';
    }
