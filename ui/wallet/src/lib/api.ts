...
export interface NetworkView {
    ...
    underwriters: number;
    /** **The ledger's record, as counts.** Contracts settled or cured (within
     *  the retention window closed rows are kept for), contracts expired and
     *  unpaid, contracts open. Community-wide and nameless: what a member
     *  weighs when deciding how far to trust a ledger, not who owes whom. */
    contracts_paid: number;
    contracts_defaulted: number;
 ...
 * e2e harnesses in `ui/wallet/scripts/`, which are not wallets and hold no member's
...
export interface TxBondView {
    ...
    /**
     * Whether the free allowance can cover this transition AT ALL — which is a
     * different question from whether any of it is left. A trade naming a key
     * with no row seats one, and a seat is never allowance-covered, so a member
     * holding every free action of the epoch is still charged for it.
     *
     * Served to a party only, and absent from a node predating it: absent
     * means unknown, and the wallet then says what it always said.
     */
    allowance_applies?: boolean;
}
