...
/** What the bond notice should say about one draft. */
export type BondNoticeState =
    /** No quote yet. Say nothing. */
    | 'unknown'
    /** The class is free for ever: settling, curing, leaving. */
    | 'always-free'
    /** The allowance covers it. */
    | 'covered'
    /** The allowance is in hand but cannot apply: this trade opens somebody's
     *  account, and a seat is never allowance-covered. */
    | 'seats'
    /** It must set aside capacity, and there is not enough free. */
    | 'short'
    /** It will set aside capacity, and there is room. */
    | 'reserves';

/**
 * **Which of those a draft is** — the third time this file has been the place
 * that found out the wallet was reading one quantity and the gate another.
 *
 * `free_remaining > 0` was taken as "this is free", and for a trade that opens
 * somebody's account it is not: the ledger consults the allowance only where
 * nothing is seated, so a member with all 32 free actions was told a seating
 * trade was free and then refused, and told to wait for a release of something
 * that was never set aside. `allowance_applies` is the ledger's own answer;
 * absent (an older node, or a read that is not a party) it is unknown, and
 * unknown keeps what the wallet said before rather than inventing a warning.
 */
export function bondNotice(quote: TxBondView | null, pos: OperationBondView | undefined): BondNoticeState {
    if (!quote) return 'unknown';
    if (quote.amount === 0) return 'always-free';
    const applies = quote.allowance_applies !== false;
    const free = pos?.free_remaining;
    const hasFree = free !== undefined && free > 0;
    if (applies && hasFree) return 'covered';
    // The binding constraint first: a member who cannot afford it needs the
    // number, not the reason it was not free.
    if (pos !== undefined && quote.amount > pos.headroom) return 'short';
    if (hasFree) return 'seats';
    return 'reserves';
}
