...
export interface AcceptancePolicy {
    ...
    background: boolean;
    /**
     * **Members whose purchases are signed on their name, not their score**,
     * each up to an amount. A regular customer whom the community has not yet
     * backed scores at the ceiling for as long as nobody has carried them —
     * which is exactly the customer a stallholder already trusts. The rule
     * cannot say that; this list can. It reaches no further than the rule's
     * other guards: the ledger's own rejection still wins, the debtor side is
     * never signed, and a request carrying a panel still waits.
     */
...
export interface TrustedMember {
    member: number;
    /** The most one of their requests may grant without being asked. */
    maxAmount: number;
}
...
export function normalizePolicy(p: Partial<AcceptancePolicy> | null | undefined): AcceptancePolicy {
    ...
    // A list read from storage is checked entry by entry: a malformed one
    // is dropped, never read as "everything" or "unlimited".
    const trusted: TrustedMember[] = Array.isArray(p?.trusted)
        ? p!.trusted!.filter(
              (t): t is TrustedMember =>
                  !!t &&
                  typeof t.member === 'number' &&
                  Number.isInteger(t.member) &&
                  t.member >= 0 &&
                  typeof t.maxAmount === 'number' &&
                  Number.isFinite(t.maxAmount) &&
                  t.maxAmount >= 0,
          )
        : [];
    return { auto, accept, reject, maxAmount, background, trusted };
}

/**
 * The ceiling a trusted member's request may reach without being asked, or
 * null when they are not on the list. A member listed twice is read at the
 * higher ceiling — the member meant to trust them, and the only question is
 * how far.
 */
export function trustedCeiling(p: AcceptancePolicy, member: number): number | null {
    let best: number | null = null;
    for (const t of p.trusted) {
        if (t.member === member && (best === null || t.maxAmount > best)) best = t.maxAmount;
    }
    return best;
}
