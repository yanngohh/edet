<script lang="ts">
    ...
    import { canSeat, seatsLeft as seatsOf } from '../lib/seats';
    ...
    $: seatsLeft = seatsOf(bondPos);
    // A newcomer's purchase seats them out of THIS member's reach. Refused
    // here, in the wallet's words, rather than at the signature the buyer
    // cannot see.
    $: noSeat = buyerId === null && !canSeat(bondPos);
...
<div class="offer-card">
    ...
    {:else}
        ...
        {#if noSeat}
            <p class="invalid" role="alert">
                {$_('offers.noSeat', {
                    default: 'Your standing carries no seat right now, so this purchase cannot open their account. Settle it in money today, or try again once somebody has backed you further.',
                })}
            </p>
        {/if}
    {/if}
    ...
    {/if}
    <div class="actions">
        <Button variant="raised" disabled={busy || noSeat} on:click={sign}>
