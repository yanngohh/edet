<script lang="ts">
    ...
    import { bondNotice } from '../lib/allowance';
    ...
    // **What to say is decided in `lib/allowance`, and tested there**, because
    // this component cannot be. Absent position (no session token yet, or a
    // viewer without full access) reads as unknown, never as zero: claiming
    // someone has no allowance left when we simply cannot see it would scare
    // them off an action that is in fact free. And holding an allowance is not
    // the same as this draft being covered by it — a trade that opens
    // somebody's account never is.
    $: state = bondNotice(bond, pos);
    $: shortfall = state === 'short';
</script>
...
{#if bond}
    <div class="bond-notice" class:short={shortfall} role="note">
        ...
        <span class="bond-text">
            ...
            {:else if state === 'seats'}
                {$_('bond.seats', {
                    values: { amount: formatAmount(formatNumber, bond.amount, $amountsSealed) },
                    default: `Your activity allowance does not cover this: a trade that opens somebody's account sets aside ${formatAmount(formatNumber, bond.amount, $amountsSealed)} of your capacity, and holds a further part of what backs you for as long as their account exists.`,
                })}
