...
// Math.random, exactly like `ui/wallet/src/lib/crypto.ts::randomNonce`) and a
// validity window. The window is read off the node's OWN current epoch,
// never a hardcoded absolute one — mirrors `ui/wallet/src/lib/submit.ts::
...
// digest it is about to sign, see `ui/wallet/src/lib/crypto.ts`'s doc comment), so
